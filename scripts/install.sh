...
# Validates install directory safety using character-set + traversal deny-list.
# Note: install.ps1 uses a different model (user-profile allowlist) because
# Windows has canonical env-var-based user directories (LOCALAPPDATA, APPDATA).
