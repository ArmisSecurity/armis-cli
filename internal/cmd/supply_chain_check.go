...
func init() {
	...
	scCheckCmd.Flags().BoolVar(&scDemo, "demo", false, "Show a demo with simulated supply chain attack detections")
...
func runSupplyChainCheck(cmd *cobra.Command, args []string) error {
	if scDemo {
		return runSupplyChainCheckDemo(cmd)
	}
...
func runSupplyChainCheckDemo(cmd *cobra.Command) error {
	...
	violations := []supplychain.Violation{
		...
		},
		{
			...
			Version:         "1.0.0",
			PublishTime:     now.Add(-6 * time.Hour),
			Age:             6 * time.Hour,
			PolicyThreshold: policy.MinReleaseAge,
			Severity:        model.SeverityHigh,
		},
	...
	fmt.Fprintf(os.Stderr, "%s %s\n",
		s.MutedText.Render("[armis]"),
		s.MutedText.Render("supply-chain: DEMO MODE — simulated supply chain attack detections"))
	fmt.Fprintf(os.Stderr, "%s %s\n\n",
		s.MutedText.Render("[armis]"),
		s.MutedText.Render(fmt.Sprintf("supply-chain: checked 47 packages, 0 skipped, %d violations (%s policy)",
			len(violations), policy.MinReleaseAge)))
	...
	fmt.Fprintf(os.Stderr, "\n%s\n",
		s.MutedText.Render("These simulated attacks (event-stream 2018, ua-parser-js 2021, typosquat 2023)"))
	fmt.Fprintf(os.Stderr, "%s\n",
		s.MutedText.Render("would all be blocked by the default 72h minimum release age policy."))
