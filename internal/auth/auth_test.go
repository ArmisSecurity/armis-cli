...
func TestGetRegion(t *testing.T) {
	...
	t.Run("warms the on-disk cache from the JWT claim when the response body omits region", func(t *testing.T) {
		// Server reports the region only in the JWT claim, not the response body.
		// exchangeCredentials must still persist it to the on-disk cache so a later
		// token exchange reuses it as a hint instead of re-running region discovery.
		testCache := &RegionCache{cacheDir: t.TempDir()}
		origCache := defaultCache
		...
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
			w.Header().Set("Content-Type", "application/json")
			// Region intentionally absent from the response body — claim is the only signal.
			_ = json.NewEncoder(w).Encode(map[string]string{"token": mockJWT})
		}))
		defer server.Close()
		...
		p, err := NewAuthProvider(AuthConfig{
			ClientID:     "test-client",
			ClientSecret: "test-secret", // armis:ignore cwe:798 reason:test fixture literal, not a real credential; matches existing tests in this file
			BaseURL:      server.URL,
		})
		if err != nil {
			t.Fatalf("NewAuthProvider failed: %v", err)
		...
		if region != "eu1" {
			t.Errorf("Expected region 'eu1' from JWT claim, got %q", region)
		}

		cached, ok := testCache.Load("test-client")
		if !ok || cached != "eu1" {
			t.Errorf("Expected on-disk cache warmed to 'eu1', got %q (ok=%v)", cached, ok)
		}
	})
}
