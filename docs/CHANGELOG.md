...
- `supply-chain init`: PowerShell profile injection is now supported. On machines where PowerShell is detected, `init` writes wrapper functions into the PowerShell profile (`~/.config/powershell/Microsoft.PowerShell_profile.ps1` on Unix, or `Documents\PowerShell\Microsoft.PowerShell_profile.ps1` on Windows). Package managers with dotted names (e.g. `pip3.12`) are skipped in the PowerShell profile — PowerShell function names may not contain dots — and a muted note explains the skip while confirming `pip` and `pip3` are still wrapped. (PPSC-1065)
